...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // 1b. Garante que o token seja de LONGA duração (60 dias) e pega a expiração
    let tokenFinal = token;
    let expiraSeg = 60 * 24 * 3600; // assume 60 dias se a troca não responder
    try {
      const longRes = await fetch(
        `https://graph.instagram.com/access_token?grant_type=ig_exchange_token&client_secret=${process.env.META_APP_SECRET}&access_token=${token}`
      );
      const t2 = await longRes.json();
      if (t2.access_token) {
        tokenFinal = t2.access_token;
        if (t2.expires_in) expiraSeg = Number(t2.expires_in);
      }
    } catch (e) { /* mantém o token original se a troca falhar */ }
    const tokenExpiraEm = new Date(Date.now() + expiraSeg * 1000).toISOString();
    // 2. Buscar dados completos do perfil
    const profRes = await fetch(`https://graph.instagram.com/v19.0/${igId}?fields=username,name,followers_count,media_count,profile_picture_url&access_token=${tokenFinal}`);
    ...
    await fetch(`${SUPABASE_URL}/rest/v1/contas_conectadas`, {
      ...
      body: JSON.stringify({ user_id: user.id, tipo: 'instagram', nome, token: tokenFinal, meta }),
    });
